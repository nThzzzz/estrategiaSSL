...
Para saber **por que** cada decisão de projeto é o que é, ou seja, por que o filtro é Kalman e
não passa-baixa, ou por que a área de defesa é cortada no `Executor` e não em cada play, o lugar
é o [README](README.md). Os dois textos têm leitores diferentes de propósito.
...
o repositório irmão. Os dois conversam só por UDP, pelo protocolo oficial da liga, e nunca por
chamada de função:
...
| **velocidade** | inferida, pelo filtro de Kalman em `percepcao/` |
| **quem está com a bola** | inferida, pela geometria da boca em `SensorDeBola` |
...
está em `src/proto/`, então um clone limpo compila **offline**.
...
> `rede.ConfigRede` com formatos diferentes, porque um descreve o mundo que *simula* e o outro o
> mundo que *observa*. São dois processos, sempre.
...
sem um software de time conectado, ninguém está mandando comando, e é assim que o grSim se
comporta também.
...
A caixa fica **cinza enquanto não há árbitro nenhum**, sem Game Controller no ar e sem o de
...
> Se descrever a coisa exige um **"e depois"**, ela não é uma `Skill`, e sim uma `Tactic`.
>
...
src/estrategia/
  ...
  ids/         os enums, e toda skill/tactic/role/play tem um id estável
...
- **`vMover` é no referencial local do robô**, ou seja, frente e esquerda, não `+x` e `+y`. Já
...
- **`bEmCampo()` antes de usar `estado()`**, porque a visão perde robô, já que oclusão acontece.
...
`estrategia` não depende de `rede` nem de `view`, e isso é verificado por teste, não é promessa.
...
`TAXA_DA_TELA` em zero, que é o padrão, pergunta ao monitor e usa a taxa dele: num monitor de
144 ou 165 Hz a janela deixa de ficar presa em 60. **Não muda a estratégia**: o `vDecidir` só age quando
...
**Os dois na mesma máquina?** Confira se o simulador não subiu com `--sem-rede`, porque assim
ele não publica nada, de propósito. No macOS, a primeira execução costuma pedir permissão de rede; se
...
> não só na padrão, e esse pedaço já era tratado aqui. O que ficou para trás foi o lado que
...
>
> Quando isso acontece, o painel avisa em amarelo: *"a visão vem de 192.168.x e os comandos vão
...
- **Sai N, chega 0**: os pacotes estão se perdendo na rede. No Windows é quase sempre o
  ...
  privadas.
- **Sai N, chega N**: chegou. O problema é outro, então siga para os itens abaixo.
